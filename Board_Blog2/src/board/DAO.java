...
public class DAO {
	...
	public void insertWrite(VO vo, int max) {
		...
		try {
			sql = "INSERT INTO board1(USERNAME,PASSWORD,TITLE,MEMO,REF,TIME,HIT,INDENT,STEP) VALUES(?,?,?,?,?,GETDATE(),0,0,0)";
			pstmt = con.prepareStatement(sql);
	...
	public void insertReply(VO vo, int ref, int indent, int step) {
		...
		try {
			sql = "INSERT INTO board1(USERNAME, PASSWORD, TITLE, MEMO, REF, INDENT, STEP,TIME,HIT) "+
					"VALUES(?,?,?,?,?,?,?,GETDATE(),0)";
			pstmt = con.prepareStatement(sql);
